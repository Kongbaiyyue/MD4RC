...
from top_model import TopModel, TopRealEstModel, TopConstractModel
...
def train(data_path, select_model, dataset_select, use_fuse_model, train_dataloader, test_dataloader, train_len, test_len, train_dataset, betas = (0.9, 0.999), lr = 0.0003, batch_size = 8, epoch = 50, l_input_dim = 13,
        ...
        fuse_ffn_dim = 128, fuse_head_size = 4, dropout = 0.1, t=1., opt_threshold = 0.1, time_t=1., model_path=None, res_path=None, use_metrics=True, use_log=True, use_softmax=True, multi_head="all_cross"):
    plan_args = Args()
    ...
    if select_model == "TopRealEstModel":
        ...
        model = TopConstractModel(t_input_dim, l_input_dim, l_hidden_dim, t_hidden_dim, emb_dim, device=device, plan_args=plan_args, sql_model=sql_model, cross_model=fuse_model, time_model=time_model)
    ...
    #         # opt_label = opt_label.masked_fill(mask_opt_label.to(device), 0)
    #         # pred_opt = pred_opt.masked_fill(mask_opt_label.to(device), 0)
    ...
    for index, input1 in enumerate(test_dataloader):
        ...
        pred_label, pred_opt_raw, _, _ = model(sql, plan, time, log)
        ...
        for l_deal_i in range(batch_size * test_idx, len_data):
            ...
            for index_pred, j_pred in enumerate(temp_list):
                if temp_list[index_pred] not in label_list[l_deal_i]:
                    label_no_pred_list.append(float(opt_label[l_deal_i - batch_size * test_idx][temp_list[index_pred]]))
            #     if temp_list[index_pred] not in label_list[l_deal_i] and opt_label[l_deal_i - batch_size * test_idx][temp_list[index_pred]] > 0.04:
            ...
            #     if label_list[l_deal_i][index_pred] not in pred_list[l_deal_i] and opt_label[l_deal_i - batch_size * test_idx][label_list[l_deal_i][index_pred]] < 0.06:
