...
def load_dataset(data_path, batch_size = 8):
    ...
    dataset = Tensor_Opt_modal_dataset()
    train_ratio = 0.7
    val_ratio = 0.15
    test_ratio = 0.15
    ...
    return train_dataloader, test_dataloader, valid_dataloader, len(train_dataset), len(test_dataset), len(val_dataset), dataset 
...
def train(data_path, select_model,  use_fuse_model, train_dataloader, test_dataloader, valid_dataloader, train_len, test_len, valid_len, train_dataset, betas = (0.9, 0.999), lr = 0.0003, batch_size = 8, epoch = 50, l_input_dim = 13,
    ...
    for i in range(epoch):
        ...
        for index, input1 in enumerate(train_dataloader):
            opt.zero_grad()
            sql, plan, time, log,opt_label,multilabel = input1

            time.to(device)
            log.to(device)
            multilabel.to(device)
            opt_label.to(device)
            ...
            if select_model == "cross_attn_no_plan":
                ...
                plan['x'] = plan['x'].squeeze(1)[:,:,0:1067].to(device).to(torch.float32)
                plan["attn_bias"] = plan["attn_bias"].squeeze(1).to(device)
                plan["rel_pos"] = plan["rel_pos"].squeeze(1).to(device)
                plan["heights"] = plan["heights"].squeeze(1).to(device)
    ...
    for index, input1 in enumerate(test_dataloader):
        sql, plan, time, log, multilabel, opt_label =input1 # input1["query"], input1["plan"], input1["timeseries"], input1["log"], input1["multilabel"], input1["opt_label"]
        sql.to(device)
        plan.to(device)
        time.to(device)
        log.to(device)
    ...
    with open(res_path, "w") as f:
        ...
        print("top 1 cor: ", top_1_cor)
        print("top 1 label: ", top_1_label)
        print("top 1 pred: ", top_1_pred)
        print("top 1 cor lable: ", top_1_cor_l)
        print("lab_cor :", lab_cor)
        print("gt_label :", gt_label)
        print("lt_label :", lt_label)

        print("MSE_loss: ", MSE_loss / float(test_len))
        print("Kendall's tau: ", evaluate_tau(label_list, pred_list))
        print("right_label_all: ", right_label_all / test_len / 5, right_label_all)
        print("top1 提升率: ", top1_valid_sum / float(top1_valid_num))
        print("pred_error_sample", pred_error_sample / test_len)
        ...
        print("top1 margin acc: ", top1_margin(label_opt_list_ndcg, pred_opt_list_ndcg))
        
        print("ndgc: ", ndcg_2(label_opt_list_ndcg, pred_opt_list_ndcg))
...
if __name__ == '__main__':
    ...
    # model_name = "GatePretrainModel" 
    model_name = "LogOptModel"
